...
// Add support for running on non-mega Arduino boards as well. #增加对 non-mega arduino 板上运行的支持
#ifdef __AVR__
...
//=============================================================================
// Global - Local to this file only...  #全局 - 仅限此文件的本地...
//=============================================================================

// definition of some helper functions  #一些辅助函数的定义
...
void ServoDriver::Init(void) {
  ...
  // Lets do the check for GP Enabled here... #让我们在这里检查GP启用...
#ifdef OPT_GPPLAYER
  char abT[4];        // give a nice large buffer.  #给一个漂亮的大缓冲区
  byte cbRead;

  _fGPEnabled = false;  // starts off assuming that it is not enabled...  #假设没有启用它
  _fGPActive = false;
  ...
  // 而不是强硬地检查版本号，而是询问其中一名玩家的状态。 如果我们没有得到回应......可能不支持
  // QPL0 /r - 查询播放器0状态,返回4字节：
  //                1.正在播放的动作组序列号（没有播放，返回255/FF）
  //                2.现在所在步数
  //                3.将要执行步数
  //                4.正在执行的动作剩余时间，单位100ms（例如剩下700ms，返回值7）
  SSCSerial.println(F("QPL0"));
  cbRead = SSCRead((byte*)abT, 4, 25000, (word) - 1);
  //  abT - 储存变量, 4 - 读取字节数,  25000 - 超时时间 , -1  -
...
//  #也许应该尽量减少当它被调用，因为它使用串行端口...
//  #也许只有当我们不插入或者如果可能一些最短时间已经过去...
//--------------------------------------------------------------------

#ifdef cVoltagePin
#ifndef CVADR1
#define CVADR1      100  // VD Resistor 1 - reduced as only need ratio... 30K and 10K  #VD电阻1  - 仅需要比例减少... 30K和10K
#define CVADR2      10  // VD Resistor 2 #VD电阻2
#endif
...
word ServoDriver::GetBatteryVoltage(void) {
  ...
  g_wVoltageSum += g_awVoltages[g_iVoltages];
  //  DBGSerial.println(g_wVoltageSum, DEC);
  return ((long)((long)g_wVoltageSum * 55 * (CVADR1 + CVADR2)) / (long)(4096 * (long)CVADR2));
}
#endif
...
//   #快速和脏的辅助函数从SSC读取这么多字节，超时和字符标记结束...
// pb - ,cb - , wTimeout - 超时时间, wEOL -
//==============================================================================
int SSCRead (byte* pb, int cb, word wTimeout, word wEOL)
{
  ...
  while (cb) {
    ...
    //#ifdef DBGSerial
    //    DBGSerial.print(F("ICH: "));
    //    DBGSerial.println(ich);
    //#endif
    ...
    *pb++ = (byte)ich;
    ...
    //#ifdef DBGSerial
    //    DBGSerial.print(F("Cb: "));
    //    DBGSerial.println(cb);
    //#endif
    //#ifdef DBGSerial
    //    DBGSerial.print(F("EOL: "));
    //    DBGSerial.println(wEOL);
    //#endif

    if ((word)ich == wEOL) {
      //#ifdef DBGSerial
      //      DBGSerial.print(F("BREAK: "));
      //      DBGSerial.println(wEOL);
      //#endif
      break;    // we matched so get out of here.  #我们匹配所以离开这里
    }

    ulTimeLastChar = micros();    // update to say we received something   #更新说我们收到了一些东西
  }
...
boolean ServoDriver::FIsGPSeqDefined(uint8_t iSeq)
{
  ...
  // See if we can see if this sequence is defined  #看看我们是否可以看到是否定义了这个序列
  ...
  return false;  // nope return error  #nope返回错误
}

//--------------------------------------------------------------------
// Setup to start sequence number...  #设置启动序列号...
//--------------------------------------------------------------------
void ServoDriver::GPStartSeq(uint8_t iSeq)
{
  if (!_fGPActive && (iSeq != 0xff)) {
    ...
    _iSeq = iSeq;  // Signal for abort  #中止信号
  }
}
...
void ServoDriver::GPPlayer(void)
{
  ...
  if (_fGPActive) {
    if (g_bGPCntSteps == 0xff) {
      // We have not init yet...  #我们尚未初始化......
      g_bGPCntSteps = GPNumSteps();  // so get the number of steps.  #所以得到步数
      if (g_bGPCntSteps == 0xff) {
        _fGPActive = false;  // error so bail out of here...  #错误所以保释出来......
      }
      else  {
        ...
        //  #由于我们正在监控GP序列，我们不再告诉它只运行一次......
        SSCSerial.print(F("PL0SQ"));
        SSCSerial.println(_iSeq, DEC);
        delay(20);
        while (SSCSerial.read() != -1)    // remove anything that was queued up.  #删除排队的任何内容
          ;
        g_InputController.AllowControllerInterrupts(true);
      }
    }
    else {
      // Player was started up, so lets see what the state is...  #玩家已经启动，所以让我们看看状态是什么......
      g_InputController.AllowControllerInterrupts(false);
      if (_iSeq = 0xff) {  // User told us to abort  #用户告诉我们要中止
      ...
      g_InputController.AllowControllerInterrupts(true);    // Ok to process hserial again...   #好的，再次处理hserial ......
    }
  }
}
...
// CommitServoDriver 更新舵机位置 - 这将输出尽可能多的命令，而无需提交它。
//           这允许我们在完成上一次更新后快速获得下一个命令
//--------------------------------------------------------------------
...
//--------------------------------------------------------------------
//[FREE SERVOS] Frees all the servos  #释放所有舵机
//--------------------------------------------------------------------
...
//      to allow it to display any additi onal commands it may have.
//    #允许终端监视器调用伺服驱动程序，以允许它显示其可能具有的任何附加命令。
//==============================================================================
...
// #终端监视器将调用此命令以查看用户输入的命令是否是伺服驱动程序添加的命令。
//==============================================================================
...
// #曾经允许像Lynxterm这样的东西通过Arduino与SSC-32交谈...会看看它是否足够快...
//==============================================================================
#ifdef OPT_SSC_FORWARDER
...
//		how well that works...
// #找出每个伺服系统的零点...将使用新的伺服功能来设置实际的pwm速率，看看它的工作情况如何...
//==============================================================================
#ifdef OPT_FIND_SERVO_OFFSETS

void FindServoOffsets()
{
  bool registerIsOK = false;   // R0 寄存器是否使能可以写入
  ...
  //-----------------------------------------------------------
  // 检查寄存器R0位是否使能脉冲偏移位 R0 = 4 即为使能
  byte r0Temp;
  byte writeT = 0;
  SSCSerial.println("R0");              // 读取R0
  delay(10);
  //  if (SSCSerial.available() > 0)
  r0Temp = (byte)SSCSerial.read();
  delay(10);
  //  Serial.print("Read R0 value:");
  //  Serial.println(r0Temp - '0');
  while (r0Temp != '4') {
    //    Serial.print("Write register: R0=4");
  ...
  if (r0Temp != '4') {
    Serial.println("Unable to write to register, Please check wiring");
    registerIsOK = false;
  } else
    registerIsOK = true;
  //-----------------------------------------------------------
  
  if (registerIsOK) {      // 寄存器使能执行
    ...
    Serial.println("Find Servo exit ");
    for (sSN = 0; sSN < 6 * NUMSERVOSPERLEG; sSN++) {
      Serial.print("Servo: ");
      Serial.print(apszLegs[sSN / NUMSERVOSPERLEG]);
      Serial.print(apszLJoints[sSN % NUMSERVOSPERLEG]);
