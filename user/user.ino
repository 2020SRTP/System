...
//[TABLES] #表
//ArcCosinus Table
//Table build in to 3 part to get higher accuracy near cos = 1.     #表格内置为3部分，以在cos = 1附近获得更高的精度
//The biggest error is near cos = 1 and has a biggest value of 3*0.012098rad = 0.521 deg.   #最大的误差是cos = 1附近 最大值为3 * 0.012098rad = 0.521度
...
// #由于表格重叠，因此不需要127 + 127 + 64的全部范围。总字节数：277
...
//Sin table 90 deg, persision 0.5 deg [180 values]  #sin 表 90度，精度0.5度[180个值]
...
//Build tables for Leg configuration like I/O and MIN/imax values to easy access values using a FOR loop  #为 I/O 和 MIN/imax 值构建 Leg 配置表，以便使用FOR循环轻松访问值
//Constants are still defined as single values in the cfg file to make it easy to read/configure  #常量仍然在cfg文件中定义为单个值，以便于读取/配置


// Servo Horn offsets  #伺服喇叭补偿
#ifdef cRRFemurHornOffset1     // per leg configuration  #每腿的配置
...
#else   // Fixed per leg, if not defined 0    #每条腿固定，如果没有定义0
...
#ifdef c4DOF
#ifdef cRRTarsHornOffset1     // per leg configuration  #每腿的配置
...
//Inverse Servo Direction #舵机方向
const bool cCoxaInv[] = {
  cRRCoxaInv, cRMCoxaInv, cRFCoxaInv, cLRCoxaInv, cLMCoxaInv, cLFCoxaInv
};
bool cFemurInv[] = {
  cRRFemurInv, cRMFemurInv, cRFFemurInv, cLRFemurInv, cLMFemurInv, cLFFemurInv
};
const bool cTibiaInv[] = {
  cRRTibiaInv, cRMTibiaInv, cRFTibiaInv, cLRTibiaInv, cLMTibiaInv, cLFTibiaInv
};
#ifdef c4DOF
...
// Define some globals for debug information #为调试信息定义一些全局变量
...
//--------------------------------------------------------------------
//[REMOTE]
#define cTravelDeadZone         4    //The deadzone for the analog input from the remote #来自遥控器的模拟输入死区
//====================================================================
//[ANGLES]
...
//--------------------------------------------------------------------
//[POSITIONS SINGLE LEG CONTROL]  - 位置单腿控制
...
//--------------------------------------------------------------------
//[OUTPUTS]
boolean         LedIKError;       //IK error output
boolean         LedIKWarning;     //IK warning output
...
// Define our global Input Control State object #定义我们的全局输入控制状态对象
INCONTROLSTATE   g_InControlState;      // This is our global Input control state object... #这是我们的全局输入控制状态对象...

// Define our ServoWriter class - 定义我们的ServoWriter类
ServoDriver  g_ServoDriver;      // our global servo driver class

boolean         g_fLowVoltageShutdown;    // If set the bot shuts down because the input voltage is to low #如果设置机器人关闭，因为输入电压太低
word            Voltage;
...
short		        NomGaitSpeed;		     //Nominal speed of the gait #步态的公称速度
short           TLDivFactor;         //Number of steps that a leg is on the floor while walking #步行时腿在地板上的步数
short           NrLiftedPos;         //Number of positions that a single leg is lifted [1-3] #单腿抬起的位置数[1-3]
byte            LiftDivFactor;       //Normaly: 2, when NrLiftedPos=5: 4 #?
byte            FrontDownPos;        //Where the leg should be put down to ground #腿应放在地上的地方

boolean         HalfLiftHeigth;      //If TRUE the outer positions of the ligted legs will be half height #如果为TRUE，则发光腿的外部位置将为半高

boolean         TravelRequest;       //Temp to check if the gait is in motion #临时检查步态是否在运动中
byte            StepsInGait;         //Number of steps in gait #步态的步数

boolean         LastLeg;             //TRUE when the current leg is the last leg of the sequence #当当前腿是序列中最后一只腿时为True
byte            GaitStep;            //Actual Gait step #实际步态

byte            GaitLegNr[6];        //Init position of the leg #初始化腿位置

byte            GaitLegNrIn;         //Input Number of the leg #输入腿数量

long            GaitPosX[6];         //Array containing Relative X position corresponding to the Gait #数组 包含与步态对应的相对X位置
...
long            GaitRotY[6];         //Array containing Relative Y rotation corresponding to the Gait #数组 包含与步态对应的相对Y旋转


boolean         fWalking;            // True if the robot are walking #如果机器人在行走，则为真
byte            bExtraCycle;         // Forcing some extra timed cycles for avoiding "end of gait bug" #强制一些额外的定时循环以避免“end of gait bug”
#define         cGPlimit 2           // GP=GaitPos testing different limits

boolean        g_fRobotUpsideDown;   // Is the robot upside down? #机器人是否倒置？
...
void setup() {
  ...
  g_fDebugOutput = false;             //  调试输出

#ifdef DBGSerial
  DBGSerial.begin(115200);
#endif

  // Init our ServoDriver  #初始化 Servo driver
  g_ServoDriver.Init();
  delay(10);
  ...
  LedIKError = 0;
  Eyes = 0;
  LedIKWarning = 0;     // No work

  //Tars Init Positions #tars 初始化位置
  for (LegIndex = 0; LegIndex <= 5; LegIndex++ )
  {
    LegPosX[LegIndex] = (short)pgm_read_word(&cInitPosX[LegIndex]);    //Set start positions for each leg  #设置每条腿的起始位置
    LegPosY[LegIndex] = (short)pgm_read_word(&cInitPosY[LegIndex]);
    LegPosZ[LegIndex] = (short)pgm_read_word(&cInitPosZ[LegIndex]);
  }

  //Single leg control. Make sure no leg is selected   #单腿控制 确保没有腿被选中
  g_InControlState.SelectedLeg = 255;      // No Leg selected  #没有腿被选中
  PrevSelectedLeg = 255;
  ...
  //Body Rotations #机体旋转参数
  g_InControlState.BodyRot1.x = 0;
  g_InControlState.BodyRot1.y = 0;
  g_InControlState.BodyRot1.z = 0;
  g_InControlState.BodyRotOffset.x = 0;
  g_InControlState.BodyRotOffset.y = 0;     //Input Y offset value to adjust centerpoint of rotation  #输入Y偏移值以调整旋转中心点
  ...
  g_InControlState.ForceGaitStepCnt = 0;    // added to try to adjust starting positions depending on height... #添加到试图根据高度调整起始位置...
  GaitStep = 1;
  GaitSelect();
...
#ifdef cVoltagePin
  pinMode(cVoltagePin, INPUT);  // VOLTAGE
  analogReference(INTERNAL);    // 基准电压 1.1V
#else
  analogReference(DEFAULT);     // 基准电压 默认5V
#endif

#ifdef DEBUG_IOPINS
  pinMode(cEyesPin, OUTPUT);          // yellow
  pinMode(cBProcessPin, OUTPUT);      // yellow
  pinMode(cIKErrorPin, OUTPUT);       // red
#endif
...
void loop(void)
{
  ...
  //Start time #开始时间
  lTimerStart = millis();
  DoBackgroundProcess();  // 后台进程
  if (!g_fLowVoltageShutdown) {
    g_InputController.ControlInput();
  }
  WriteOutputs();         // Write Outputs #写输出

#ifdef IsRobotUpsideDown  // 机器人颠倒了
  if (!fWalking) {        // dont do this while walking #机体运行时不要做这个
    g_fRobotUpsideDown = IsRobotUpsideDown;             // Grab the current state of the robot... #机器人当前状态
    if (g_fRobotUpsideDown != fRobotUpsideDownPrev) {
      // Double check to make sure that it was not a one shot error #仔细检查以确保它不是一次性错误
      g_fRobotUpsideDown = IsRobotUpsideDown;           // Grab the current state of the robot... #抓住机器人的当前状态...
      if (g_fRobotUpsideDown != fRobotUpsideDownPrev) {
...
#ifdef OPT_WALK_UPSIDE_DOWN   // 行走方向颠倒
...
#ifdef OPT_GPPLAYER   // GP动作组 播放器
  //GP Player #GP 播放器
  g_ServoDriver.GPPlayer();
  if (g_ServoDriver.FIsGPSeqActive())
    return;  // go back to process the next message #返回处理下一条消息
#endif

  //Single leg control #单引脚控制
  SingleLegControl ();
  DoBackgroundProcess();
  ...
  //Balance calculations #平衡计算
  TotalTransX = 0;     //reset values used for calculation of balance #重置值
  TotalTransZ = 0;
  TotalTransY = 0;
  TotalXBal1 = 0;
  TotalYBal1 = 0;
  ...
  if (g_InControlState.BalanceMode) {
    for (LegIndex = 0; LegIndex <= 2; LegIndex++) {    // balance calculations for all Right legs #所有右腿的平衡计算
      DoBackgroundProcess();
    ...
    for (LegIndex = 3; LegIndex <= 5; LegIndex++) {    // balance calculations for all Left legs #所有左腿的平衡计算
      DoBackgroundProcess();
      BalCalcOneLeg(LegPosX[LegIndex] + GaitPosX[LegIndex],
                    LegPosZ[LegIndex] + GaitPosZ[LegIndex],
                    (LegPosY[LegIndex] - (short)pgm_read_word(&cInitPosY[LegIndex])) + GaitPosY[LegIndex], LegIndex);
  ...
  //Reset IKsolution indicators #重置IKsolution指标
  IKSolution = 0 ;
  IKSolutionWarning = 0;
  IKSolutionError = 0 ;

  //Do IK for all Right legs #IK 右腿
...
#ifdef OPT_WALK_UPSIDE_DOWN
  if (g_fRobotUpsideDown) {         //Need to set them back for not messing with the SmoothControl #需要将它们设置为不干扰SmoothControl
    g_InControlState.BodyPos.x = -g_InControlState.BodyPos.x;
    g_InControlState.SLLeg.x = -g_InControlState.SLLeg.x;
    g_InControlState.BodyRot1.z = -g_InControlState.BodyRot1.z;
  }
  ...
  //Check mechanical limits #检查机器限制
  CheckAngles();

  //Write IK errors to leds #指示灯
  LedIKError = IKSolutionError;
  LedIKWarning = IKSolutionWarning;
  ...
  if (g_InControlState.fHexOn) {
    ...
    //Calculate Servo Move time #计算舵机移动时间
    if ((abs(g_InControlState.TravelLength.x) > cTravelDeadZone) || (abs(g_InControlState.TravelLength.z) > cTravelDeadZone) ||
      ...
      //Add aditional delay when Balance mode is on #平衡模式打开时添加额外延迟
      if (g_InControlState.BalanceMode)
        ServoMoveTime = ServoMoveTime + 100;
    }
    else //Movement speed excl. Walking #移动速度不包括 步行
    ...
    // 注意我们将伺服驱动器分解为启动/提交，这样我们可以在等待之前输出所有伺服信息，并且在等待之后只有终止信息输出。这样我们希望我们的时间更精确......
    DoBackgroundProcess();
    StartUpdateServos();
    ...
    // 看看我们是否需要在步行时同步我们的处理器和伺服驱动器，以确保在发送下一个之前prev已完成
    ...
    for (LegIndex = 0; LegIndex <= 5; LegIndex++) {
      if ( (GaitPosX[LegIndex] > cGPlimit) || (GaitPosX[LegIndex] < -cGPlimit)
        ...
        bExtraCycle = NrLiftedPos + 1;    // For making sure that we are using timed move until all legs are down  为了确保我们正在使用定时移动，直到所有腿都停下来
        break;
      }
    }
    ...
    //Turn the bot off - May need to add ajust here...  #关闭机器人 - 可能需要在此处添加调整...
    ...
    // We also have a simple debug monitor that allows us to check things. call it here..
    // 我们也有一个简单的调试监视器，可以让我们检查事情。 这里调用它..
#ifdef OPT_TERMINAL_MONITOR
    if (TerminalMonitor())
      return;
#endif
    delay(20);  // give a pause between times we call if nothing is happening - 如果什么事情都没有发生，我们就会打电话给我们暂停
  }

  PrevServoMoveTime = ServoMoveTime;

  //Store previous g_InControlState.fHexOn State - 存储以前的g_InControlState.fHexOn状态
  if (g_InControlState.fHexOn)
...
void StartUpdateServos() {
  ...
  for (LegIndex = 0; LegIndex <= 5; LegIndex++) {
    // Inverse Servo Direction #舵机方向判定
#ifdef c4DOF
...
//--------------------------------------------------------------------
//[WriteOutputs] Updates the state of the leds #更新LED的状态
//--------------------------------------------------------------------
void WriteOutputs(void) {
#ifdef DEBUG_IOPINS
  digitalWrite(cEyesPin, Eyes);
  digitalWrite(cIKErrorPin, LedIKError);
#endif
}

#define CHECKVOLTAGEONE  // 使用方法一测量则定义，方法二则注释（两个方法一样效果）
//--------------------------------------------------------------------
//[CHECK VOLTAGE - 检查电压]
...
boolean CheckVoltage() {
  ...
  if (!g_fLowVoltageShutdown) {
    if ((Voltage < cBuzzerAlarmVol) || (Voltage >= 1999)) {
#ifdef DBGSerial
      DBGSerial.print("Voltage went low, buzzer alarm ");
      DBGSerial.println(Voltage, DEC);
#endif //DBGSerial
      MSound(1, 20, 1000);  // Buzzer alarm
    }
  }
#endif //cBuzzerAlarmVol
...
void GaitSelect(void)
{
  ...
  switch (g_InControlState.GaitType)  {
    ...
    case 2:
      ...
      break;
    case 3:
      // Triple Tripod 16 steps, use 5 lifted positions - 三脚架16步，使用5个提升位置
      GaitLegNr[cRF] = 4;
      GaitLegNr[cLM] = 5;
      GaitLegNr[cRR] = 6;
      GaitLegNr[cLF] = 12;
...
void GaitSeq(void)
{
  //Check if the Gait is in motion - 检查步态是否在运动中
  ...
  //Calculate Gait sequence - 计算步态序列
  LastLeg = 0;
  ...
  // If we have a force count decrement it now... -  数量递减
...
{
  //Clear values under the cTravelDeadZone - 清除cTravelDeadZone下的值
  if (!TravelRequest) {
    g_InControlState.TravelLength.x = 0;
    g_InControlState.TravelLength.z = 0;
    g_InControlState.TravelLength.y = 0; //Gait NOT in motion, return to home position-不在运行中，返回原位
  }
  //Leg middle up position OK - 腿中间位置可以
  //Gait in motion -  运行中
  ...
  //Optional Half heigth Rear (2, 3, 5 lifted positions) - 可选半高后部（2,3,5提升位置）
  ...
  // _A_
  // Optional Half heigth front (2, 3, 5 lifted positions) - 可选半高前（2,3,5提升位置）
  ...
  //Optional Half heigth Rear 5 LiftedPos (5 lifted positions) - 可选半高后5提升位置（5提升位置）
  ...
  //Optional Half heigth Front 5 LiftedPos (5 lifted positions) - 可选半高前部5提升位置（5提升位置）
...
void BalCalcOneLeg (long PosX, long PosZ, long PosY, byte BalLegNr)
{
  ...
  //Calculating totals from center of the body to the feet - 计算从身体中心到脚的总计
...
//--------------------------------------------------------------------
//(BODY INVERSE KINEMATICS)  - 身体反转运动学
//BodyRotX         - Global Input pitch of the body           - 俯仰
//BodyRotY         - Global Input rotation of the body        - 旋转
//BodyRotZ         - Global Input roll of the body            - 滚动
//RotationY        - Input Rotation for the gait              - 输入步态旋转
//PosX             - Input position of the feet X             - 输入脚X 的位置
//PosZ             - Input position of the feet Z             - 输入脚Z 的位置
//SinB             - Sin buffer for BodyRotX                  - BodyRotX sin 缓存
//CosB             - Cos buffer for BodyRotX                  - BodyRotX cos 缓存
//SinG             - Sin buffer for BodyRotZ                  - BodyRotZ sin 缓存
//CosG             - Cos buffer for BodyRotZ                  - BodyRotZ cos 缓存
//BodyFKPosX       - Output Position X of feet with Rotation  - 带旋转的脚的输出位置X.
//BodyFKPosY       - Output Position Y of feet with Rotation  - 带旋转的脚的输出位置Y.
//BodyFKPosZ       - Output Position Z of feet with Rotation  - 带旋转的脚的输出位置Z.
...
// 计算脚的 coxa、femur、tibia 对于给定位置的角度
//IKFeetPosX            - Input position of the Feet X - 脚X的输入位置
//IKFeetPosY            - Input position of the Feet Y
//IKFeetPosZ            - Input Position of the Feet Z
//IKSolution            - Output true if the solution is possible          - 如果解决方案可行，则输出true
//IKSolutionWarning     - Output true if the solution is NEARLY possible   - 如果解决方案几乎可能，则输出true
//IKSolutionError       - Output true if the solution is NOT possible      - 如果解决方案不可行，则输出true
//FemurAngle1           - Output Angle of Femur in degrees   - Femur 输出角度（度）
//TibiaAngle1           - Output Angle of Tibia in degrees   - Tibia 输出角度（度）
//CoxaAngle1            - Output Angle of Coxa in degrees    - Coxa  输出角度（度）
//--------------------------------------------------------------------
void LegIK (short IKFeetPosX, short IKFeetPosY, short IKFeetPosZ, byte LegIKLegNr) {
  unsigned long    IKSW2;            //Length between Shoulder and Wrist, decimals = 2 - 肩与腕之间的长度，小数= 2
...
#define TarsOffsetY  0		// Vector value / The 2 DOF IK calcs (femur and tibia) are based upon these vectors
  ...
  //Using GetAtan2 for solving IKA1 and IKSW #使用GetAtan2来解决IKA1和IKSW
  //IKA14 - Angle between SW line and the ground in radians #IKA14 - SW线与地面之间的角度，弧度
  IKA14 = GetATan2 (IKFeetPosY - TarsOffsetY, IKFeetPosXZ - (byte)pgm_read_byte(&cCoxaLength[LegIKLegNr]) - TarsOffsetXZ);

  //IKSW2 - Length between femur axis and tars  #IKSW2 - 股骨轴和胫骨之间的长度
  IKSW2 = XYhyp2;
...
//--------------------------------------------------------------------
//[CHECK ANGLES] Checks the mechanical limits of the servos #检查舵机的机械限制
//--------------------------------------------------------------------
...
//--------------------------------------------------------------------
// SmoothControl (From Zenta)  - 平滑控制
//  - This function makes the body rotation and translation much smoother
//  - 此功能可使身体旋转和平移更平滑
//--------------------------------------------------------------------
...
//    快速和肮脏的音调功能试图向扬声器输出一些频率以获得一些简单的声音。
//==============================================================================
#ifdef SOUND_PIN
...
//==============================================================================
// TerminalMonitor - 终端监视器
//   - Simple background task checks to see if the user is asking
//    us to do anything, like update debug levels ore the like.
//  -简单的后台任务检查以查看用户是否要求我们执行任何操作，例如更新调试级别等。
//==============================================================================
boolean TerminalMonitor(void) {
  byte szCmdLine[20];  // currently pretty simple command lines...
  byte ich;
  int ch;
  // See if we need to output a prompt. #看看我们是否需要输出提示。
  if (g_fShowDebugPrompt) {
    ...
    // Let the Servo driver show it's own set of commands... #让伺服驱动程序显示它自己的一组命令...
    g_ServoDriver.ShowTerminalCommandList();
    g_fShowDebugPrompt = false;
  }

  // First check to see if there is any characters to process. #首先检查是否有任何字符需要处理。
  if ((ich = DBGSerial.available())) {
    ich = 0;
    // For now assume we receive a packet of data from serial monitor, as the user has to click the send button...
    // #现在假设我们收到来自串行监视器的数据包，因为用户必须单击发送按钮...
    ...
    szCmdLine[ich] = '\0';    // go ahead and null terminate it... #继续并终止它...
    DBGSerial.print(F("Serial Cmd Line:"));
    DBGSerial.write(szCmdLine, ich);
    DBGSerial.println(F("!!!"));

    // So see what are command is. #所以看看命令是什么
    if (ich == 0) {
      g_fShowDebugPrompt = true;
...
void DumpEEPROMCmd(byte * pszCmdLine) {
  // first byte can be H for hex or W for words...  #第一个字节可以是H表示十六进制或W表示单词...
  if (!*++pszCmdLine)  // Need to get past the command letter first...  #需要先通过命令信...
    DumpEEPROM();
    ...
    // First argument should be the start location to dump #第一个参数应该是转储的起始位置
    g_wEEPromDumpStart = GetCmdLineNum(&pszCmdLine);

    // If the next byte is an "=" may try to do updates... #如果下一个字节是“=”可能会尝试进行更新...
    if (*pszCmdLine == '=') {
      // make sure we don't get stuck in a loop... #确保我们不会陷入循环......
      byte *psz = pszCmdLine;
      word w;
      while (*psz) {
        w = GetCmdLineNum(&psz);
